...
function generateCloudInit(
  ...
  // - OpenRouter: "openrouter/anthropic/claude-sonnet-4.5" (dots in version, openrouter/ prefix)
  // - Direct BYOK: "anthropic/claude-sonnet-4-5" (dashes in version, provider/ prefix)
  // The UI stores model IDs with dots (e.g. "claude-sonnet-4.5") but OpenClaw's direct
  // provider registry uses dashes (e.g. "anthropic/claude-sonnet-4-5").
  const model = agentConfig.llmModel ?? (isOpenRouter ? "anthropic/claude-sonnet-4.5" : "gpt-4.1-mini");
  const openclawModel = isOpenRouter
    ? `openrouter/${model}`
    : `${provider}/${model.replace(/(\d+)\.(\d+)/g, "$1-$2")}`;
  ...
  if (isOpenRouter) {
    ...
    // Both API keys and setup-tokens are set as env vars.
    // Setup-tokens (e.g. from `claude setup-token`) are functionally API keys
    // and work as ANTHROPIC_API_KEY. The `openclaw models auth paste-token` CLI
    // command is interactive (TUI) and fails in non-interactive cloud-init.
...
export async function createServerForInstance(instanceId: string): Promise<void> {
  ...
  try {
    ...
    // Look up user's LLM API key (or setup-token) from integrations table
    ...
    if (llmProvider && isByokProvider(llmProvider)) {
      ...
      if (llmIntegration?.accessToken) {
        try {
          ...
          const authMethod = (llmIntegration.metadata as Record<string, unknown>)?.authMethod as string ?? "api-key";
          console.log(`[Hetzner] Found BYOK ${authMethod} for provider ${llmProvider}`);
        } catch (err) {
