...
describe("Calendly OAuth Integration", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  });

  describe("GET /api/integrations/calendly", () => {
    ...
    it("should redirect to Calendly OAuth URL when authenticated", async () => {
      ...
      expect(location).toContain("auth.calendly.com/oauth/authorize");
  ...
  describe("GET /api/integrations/calendly/callback", () => {
    it("should redirect with error if OAuth was denied", async () => {
      ...
      const request = new Request(
        "http://localhost:3000/api/integrations/calendly/callback?error=access_denied"
      ) as any;
    ...
    it("should redirect with error if state validation fails", async () => {
      ...
      const { GET: CallbackGET } = await import("../callback/route");
      const request = new Request(
        "http://localhost:3000/api/integrations/calendly/callback?code=test-code&state=invalid-state"
      ) as any;
      request.nextUrl = new URL(request.url);
