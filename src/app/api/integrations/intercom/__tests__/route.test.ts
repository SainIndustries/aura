...
describe("Intercom OAuth Integration", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  });

  describe("GET /api/integrations/intercom", () => {
    ...
    it("should redirect to integrations with error if client ID is not configured", async () => {
      ...
      expect(response.headers.get("location")).toContain(
        "error=intercom_not_configured"
      );
    });

    it("should redirect to Intercom OAuth URL when authenticated", async () => {
      vi.stubEnv("INTERCOM_CLIENT_ID", "test-client-id");
      vi.stubEnv("NEXT_PUBLIC_APP_URL", "http://localhost:3000");
      vi.mocked(getCurrentUser).mockResolvedValue({ id: "user-1" } as any);
      ...
      expect(location).toContain("app.intercom.com/oauth");
      expect(location).toContain("client_id=test-client-id");
      expect(location).toContain("state=test-state");
    });
  });

  describe("GET /api/integrations/intercom/callback", () => {
    it("should redirect with error if OAuth was denied", async () => {
      vi.stubEnv("NEXT_PUBLIC_APP_URL", "http://localhost:3000");

      const { GET: CallbackGET } = await import("../callback/route");
      const request = new Request(
        "http://localhost:3000/api/integrations/intercom/callback?error=access_denied"
      ) as any;
    ...
    it("should redirect with error if state validation fails", async () => {
      ...
      const { GET: CallbackGET } = await import("../callback/route");
      const request = new Request(
        "http://localhost:3000/api/integrations/intercom/callback?code=test-code&state=invalid-state"
      ) as any;
      request.nextUrl = new URL(request.url);
