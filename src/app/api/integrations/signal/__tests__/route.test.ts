...
describe("Signal API Key Integration", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockFetch.mockReset();
  });

  describe("GET /api/integrations/signal", () => {
    ...
    it("should return connected: false if no integration exists", async () => {
      ...
      expect(data.provider).toBe("signal");
    });
    ...
    it("should return connected: true if integration exists", async () => {
      ...
      vi.mocked(db.query.integrations.findFirst).mockResolvedValue({
        ...
        metadata: { baseUrl: "http://localhost:8080", phoneNumber: "+1234567890" },
      } as any);
  ...
  describe("POST /api/integrations/signal", () => {
    it("should return 400 if API key is missing", async () => {
      vi.mocked(getCurrentUser).mockResolvedValue({ id: "user-1" } as any);

      const request = new Request("http://localhost:3000/api/integrations/signal", {
        method: "POST",
    ...
    it("should return 400 if phone number is missing", async () => {
      ...
      expect(response.status).toBe(400);
      expect(data.error).toBe("Phone number is required for Signal");
    });
  ...
  describe("DELETE /api/integrations/signal", () => {
