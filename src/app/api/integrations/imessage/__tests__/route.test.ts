...
describe("iMessage API Key Integration", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockFetch.mockReset();
  });

  describe("GET /api/integrations/imessage", () => {
    ...
    it("should return connected: false if no integration exists", async () => {
      ...
      expect(data.provider).toBe("imessage");
    });
    ...
    it("should return connected: true if integration exists", async () => {
      ...
      vi.mocked(db.query.integrations.findFirst).mockResolvedValue({
        ...
        metadata: { baseUrl: "http://localhost:1234" },
      } as any);
      mockFetch.mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({ data: { server_name: "BlueBubbles" } }),
      });
  ...
  describe("POST /api/integrations/imessage", () => {
    it("should return 400 if API key is missing", async () => {
      vi.mocked(getCurrentUser).mockResolvedValue({ id: "user-1" } as any);

      const request = new Request("http://localhost:3000/api/integrations/imessage", {
        method: "POST",
        body: JSON.stringify({}),
      });
  ...
  describe("DELETE /api/integrations/imessage", () => {
